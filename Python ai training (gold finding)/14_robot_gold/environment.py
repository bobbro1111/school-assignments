...
class RobotWorld:
    ...
    def cost(self,solution):
    	# >>>>>>> COMPLETE THIS 
        # Change cost variables
        # 
        self.robot_location = copy.copy(self.robot_start_location)
        move_count = 0
        cost = 0
        dead_cost = 1000 * (move_count+1) #Helps with early deaths?
        gold_cost = -20 #Rewards finding gold :)
        
        for mov in solution:
            move_count += 1
            move_cost = move_count * 4 #if we don't find gold the cost is 5 times move count
