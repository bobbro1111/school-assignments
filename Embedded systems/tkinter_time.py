...
class TimeApp:
    ...
    def update_display(self):
        self.hours_label.config(text=self.convert(self.hours))
        self.colon_label.config(text=":")
        self.minutes_label.config(text=self.convert(self.minutes))
    ...
    def plus_pressed(self):
        """Increment hours or minutes based on current mode"""
        if self.state == ADJUST_HOUR:
            self.hours = (self.hours + 1) % 24
        elif self.state == ADJUST_MINUTE:
            self.minutes = (self.minutes+1) % 60
        self.update_display()

    def minus_pressed(self):
        """Decrement time or minutes based on current mode"""
    ...
    def time_pressed(self):
        ...
        if self.state == ADJUST_HOUR:
            # Red LED for hour edit, green LED for minute idle
            self.hours_label.config(fg="#FF0000")
            self.minutes_label.config(fg="#00FF00")
        elif self.state == ADJUST_MINUTE:
            # Green LED for hour idle, red LED for minute edit
            self.hours_label.config(fg = "#00FF00")
            self.minutes_label.config(fg = "#FF0000")
        else:
            # back to original dark-green display
            self.hours_label.config(fg ="#3A5A40")
            self.minutes_label.config(fg = "#3A5A40")
